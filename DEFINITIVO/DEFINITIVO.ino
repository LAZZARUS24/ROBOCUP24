...
#define triggerPin2 35 //sinistra
#define echoPin2 34
...
void setup() {
 ...
 pinMode(enA, OUTPUT);
  ...
  pinMode(23, INPUT);     //encoder
  // Esegui prima lettura dei valori sui pin
  prevClk = digitalRead(4);
  tic = 0;
  analogWrite(enA, 200);
  analogWrite(enB, 200);
}
...
//*********************************************************************************************************
  ...
  Serial.println(status);
 // while(status!=5){} // stop everything if could not connect to MPU6050
